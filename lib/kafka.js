...
class KafkaWorker {
   ...
   * @return {undefined}
   */
  comsumerHandler(handler) {
    const getCacheLength = consumer => {
      return Object.keys(consumer.cache).length;
    };
    this.consumers.forEach(consumer => {
      consumer.on('message', async message => {
        const traceId = uuid.v1();
        message.traceId = traceId;
        try {
          consumer.cache[traceId] = message;
          let cacheLength = getCacheLength(consumer);
          this.logger.info(`received message, cache: ${cacheLength}, partition: ${message.partition}, offset: ${message.offset}`);
          if (cacheLength >= 10000 && !consumer.paused) {
            consumer.pause();
            this.logger.info('consumer paused');
          }
          consumer.cache[traceId] = message;
          await handler(message);
          delete consumer.cache[traceId];
          cacheLength = getCacheLength(consumer);
          this.logger.info(`resolved message, cache: ${cacheLength}, partition: ${message.partition}, offset: ${message.offset}`);
          if (cacheLength < 10000 && consumer.paused) {
            consumer.resume();
            this.logger.info('consumer resume');
          }
        } catch (err) {
          this.logger.error(`[comsumerHandler] [${message.traceId}] - dpc handler error,`, err, '\nMessage:\n', message);
        }
      });

      consumer.on('error', err => {
        this.logger.error('[comsumerHandler] - consumer error,\nError:', err);
      });
    });
  }
  ...
  async validateGroups(consumerGroups) {
    ...
    assert(
      ...
      `[validataGroups] - groupId错误，定义：${groupId}, 实际：${consumerOptions.groupId}`
    );
    this.logger.info(
      `[validataGroups] - consumerGroupId: ${groupId}, consumerGroupLength: ${groupLength}, newAdded: ${this.consumers.length}`
    );
  }
    ...
    const createGroup = async () => {
      ...
      return new Promise(resolve => {
        producer.once('ready', () => {
          ...
          this.producers.availables[producer.producerId] = producer;
  ...
  sendMsg(payloads, traceId) {
    ...
    const keysArray = Object.keys(producers.availables); // 获取producer keys
    const key = keysArray[parseInt(Math.random() * keysArray.length)]; // Random获取key
    const producer = producers.availables[key]; // 获取producer
