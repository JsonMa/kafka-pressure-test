...
class KafkaWorker {
  ...
  consumerHandler(handler) {
    this.consumers.forEach(consumer => {
      consumer.asyncQueue = async.queue(async message => {
        async () => {
          try {
            await handler(message);
            this.logger.info(`resolved message, cache: ${consumer.asyncQueue.length()}, partition: ${message.partition}, offset: ${message.offset}`);
          } catch (err) {
      ...
      consumer.on('message', message => {
        ...
        consumer.asyncQueue.push(message, () => {
          this.logger.info(`received message, cache: ${consumer.asyncQueue.length()}, partition: ${message.partition}, offset: ${message.offset}`);
        });
      });
